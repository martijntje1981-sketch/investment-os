-- Phase 1: deterministic ledger transaction ordering
-- Root cause fix: rows inserted in the same transaction share created_at (transaction
-- timestamp) and often share executed_at (calendar date). UUID id tie-breaking was
-- non-deterministic and could process sells before buys.

BEGIN;

CREATE SEQUENCE IF NOT EXISTS public.transactions_ledger_sequence_seq;
...
SET ledger_sequence = ordered.rn
FROM ordered
WHERE ordered.id = txn.id
  AND txn.ledger_sequence IS NULL;

SELECT pg_catalog.setval(
  'public.transactions_ledger_sequence_seq',
  COALESCE((SELECT pg_catalog.max(ledger_sequence) FROM public.transactions), 0),
  COALESCE((SELECT pg_catalog.max(ledger_sequence) FROM public.transactions), 0) > 0
);

ALTER TABLE public.transactions
  ALTER COLUMN ledger_sequence SET DEFAULT pg_catalog.nextval('public.transactions_ledger_sequence_seq');

ALTER TABLE public.transactions
  ALTER COLUMN ledger_sequence SET NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS transactions_ledger_sequence_idx
  ON public.transactions (ledger_sequence);

DROP INDEX IF EXISTS public.transactions_holding_executed_idx;

CREATE INDEX transactions_holding_executed_idx
  ON public.transactions (holding_id, executed_at, ledger_sequence);

GRANT USAGE, SELECT ON SEQUENCE public.transactions_ledger_sequence_seq TO authenticated, service_role;
