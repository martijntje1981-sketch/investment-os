/**
 * Run the A1 NAV snapshot migration against a disposable Postgres.
 * Modes:
...
const container = "ios-nav-a1-disposable-pg";
const postgresImage = "postgres:15-alpine";

const sqlFiles = [
  "supabase/tests/database/portfolio_nav_snapshots_disposable_stub.sql",
  "supabase/migrations/20260901120000_portfolio_nav_snapshots.sql",
  "supabase/tests/database/portfolio_nav_snapshots_disposable_verify.sql",
];
...
function runAgainstDatabaseUrl(databaseUrl, target) {
  ...
  console.log("Disposable Postgres verification passed.");
  console.log(
    "Note: auth.uid() in this harness reads GUC app.user_id, not a Supabase JWT. RLS using user_id = auth.uid() is otherwise equivalent.",
  );
  console.log(
    "Note: unavailable/invalid NAV rows are rejected by the Goal Pace writer tests before SQL write; usability has no 'unavailable' value.",
  );
}
